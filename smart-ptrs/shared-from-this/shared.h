...
public:
    ...
    explicit SharedPtr(T* ptr) {
        ...
        if constexpr (std::is_convertible_v<T*, EFSCBase*>) {
            if (ptr) {
                ptr->data = block;
            }
        }
    ...
    SharedPtr(ControlBlock* other_block) {
        block = other_block;
    }
...
SharedPtr<T> MakeShared(Args&&... args) {
    ...
    if constexpr (std::is_convertible_v<T*, EFSCBase*>) {
        auto p = static_cast<T*>(block->Get());
        p->data = block;
    }
...
template <typename T>
class EnableSharedFromThis : public EFSCBase {
public:
